...
void Model::loadMesh(unsigned int meshIndex)
{
  ...
  unsigned int indiceAccIndex{JSON["meshes"][meshIndex]["primitives"][0]["indices"]};

  std::vector<float> posVec{getFloats(JSON["accessors"][positionAccIndex])};
  std::vector<glm::vec3> positions{groupFloatsVec3(posVec)};
  std::vector<float> normalVec{getFloats(JSON["accessors"][normalAccIndex])};
  std::vector<glm::vec3> normals{groupFloatsVec3(normalVec)};
  std::vector<float> texVec{getFloats(JSON["accessors"][textureAccIndex])};
  std::vector<glm::vec2> texUVs{groupFloatsVec2(texVec)};

  std::vector<Vertex> vertices{assembleVertices(positions, normals, texUVs)};
  std::vector<GLuint> indices{getIndices(JSON["accessors"][indiceAccIndex])};
...
std::vector<float> Model::getFloats(json accessor)
{
  ...
  if (type == "SCALAR")
    ...
    throw std::invalid_argument("Invalid type (not SCALAR, VEC2, VEC3, VEC4)");
...
std::vector<Texture> Model::getTextures()
{
  ...
  for (unsigned int i{0}; i < JSON["images"].size(); i++)
  {
    ...
    {
      if (loadedTexturesName[j] == texturePath)
      {
    ...
    if (!skip)
    {
      ...
      {
        Texture diffuse((fileDirectory + texturePath).c_str(), "diffuse", loadedTextures.size());
        textures.push_back(diffuse);
        loadedTextures.push_back(diffuse);
        loadedTexturesName.push_back(texturePath);
      }
      else if (texturePath.find("metallicRoughness") != std::string::npos || texturePath.find("specular") != std::string::npos)
      {
        Texture specular((fileDirectory + texturePath).c_str(), "specular", loadedTextures.size());
